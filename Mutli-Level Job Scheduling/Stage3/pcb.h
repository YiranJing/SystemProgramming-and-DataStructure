...
struct pcb {
    ...
    int mbytes; // the memeory requirement
    MabPtr memoryblock;
    int status;
    struct pcb * next;
};
...
PcbPtr findMaxRemainingTime(PcbPtr); // find the job with the longest remaining exection time
PcbPtr removeSpecificJob(PcbPtr, PcbPtr);// remove specific job based on pid
