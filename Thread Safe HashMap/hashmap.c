...
/*
    ...
    Lock the whole map when expend
*/
void expandIfNecessary(struct hash_map* map){
    ...
    init_buckets(new_bucket, new_n_buckets); // create empty new hashtable
	...
	for (size_t i = 0; i<map->n_buckets; i++) {
        ...
        struct node *next = n->next;
			int hash = map->hash(n->k); //remap hash table based on the new hashvalue
...
/*
  Helper function of hash_map_remove_entry
...
void hash_map_remove_entry(struct hash_map* map, void* k) {
    ...
    if (c == 1) { // should remobe the first node of the linkedlist
